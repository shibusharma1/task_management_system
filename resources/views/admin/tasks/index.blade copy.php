            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">Task</th>
                        ...
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">Assignee</th>
            ...
            <form id="task-form" method="POST" class="space-y-4" action="{{ route('tasks.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="assigned_by_search" placeholder="Search user by name or email" class="mt-1 block w-full border-gray-300 rounded-md px-3 py-2" value="{{ auth()->user()->name }}" disabled>
                        <div id="assigned_by_suggestions" class="bg-white border mt-1 rounded shadow max-h-48 overflow-auto hidden"></div>
                    </div>

                    <div class="md:col-span-2">
                        <div class="flex items-center space-x-3">
                            <input type="checkbox" id="is_requested" name="is_requested" class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
                            <label for="is_requested" class="text-sm text-gray-700">Is Requested</label>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const statusInput = document.getElementById('task-status') || { value: 0 };
    const approvedInput = document.getElementById('task-approved') || { value: 0 };
    const categorySelect = document.getElementById('task_category_id');
    const prioritySelect = document.getElementById('priority_id');
    const isRequestedCheckbox = document.getElementById('is_requested');
    ...
    const assignedBySearch = document.getElementById('assigned_by_search');
    const assignedByHidden = document.getElementById('assigned_by');
    const assignedBySuggestions = document.getElementById('assigned_by_suggestions');
    ...
    if (newBtn) {
        newBtn.addEventListener('click', () => {
            ...
            assignedByHidden.value = '';
            assignedBySearch.value = '';
            isRequestedCheckbox.checked = false;
    ...
    document.querySelectorAll('.view-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(`{{ url('admin/tasks') }}/${id}`, {
                ...
                statusInput.value = task.status ?? 0;
                approvedInput.value = task.is_approved ?? 0;
                ...
                assignedByHidden.value = task.assigned_by ?? '';
                assignedBySearch.value = task.requester ? `${task.requester.name} <${task.requester.email}>` : '';
                isRequestedCheckbox.checked = !!task.is_requested;
    ...
    assignedBySearch.addEventListener('input', debounce(async () => {
        const users = await searchUsers(assignedBySearch.value);
        showSuggestions(assignedBySuggestions, users);
    }));

    assignedBySuggestions.addEventListener('click', e => {
        if (e.target.dataset.id) {
            assignedByHidden.value = e.target.dataset.id;
            assignedBySearch.value = e.target.innerText;
            assignedBySuggestions.classList.add('hidden');
        }
    });
