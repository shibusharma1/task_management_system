...
<div class="hidden md:flex md:flex-shrink-0">
    <div class="flex flex-col w-64 bg-white border-r border-gray-200">
        ...
        <div class="space-y-1">
          ...
          <div x-data="{ open: {{ request()->routeIs('task.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse class="ml-6 mt-2 space-y-1">
              <a href="{{ route('task.index') }}" class="flex items-center px-2 py-2 text-sm rounded-md
              ...
              <a href="{{ route('task.mytask') }}" class="flex items-center px-2 py-2 text-sm rounded-md
              ...
              <a href="{{ route('task.priority') }}" class="flex items-center px-2 py-2 text-sm rounded-md
              ...
              <a href="{{ route('task.category') }}" class="flex items-center px-2 py-2 text-sm rounded-md
          ...
          <a href="#" 
            ...
            <i class="fas fa-chart-line mr-3 {{ request()->routeIs('performance.*') ? 'text-indigo-500' : 'text-gray-600' }}"></i>
            Performance
          </a>
          ...
          <div x-data="{ open: {{ request()->routeIs('settings.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse class="ml-6 mt-2 space-y-1">
              <a href="{{ route('settings.general') }}" class="flex items-center px-2 py-2 text-sm rounded-md
              ...
              <a href="{{ route('settings.institutions') }}" class="flex items-center px-2 py-2 text-sm rounded-md
          ...
          <div class="pt-4 mt-4 border-t border-gray-200" x-data="{ open: {{ request()->routeIs('users.*') || request()->routeIs('designations.*') || request()->routeIs('departments.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" class="ml-6 mt-2 space-y-1" x-cloak>
              <a href="{{ route('users.index') }}" class="flex items-center px-2 py-2 text-sm rounded-md {{ request()->routeIs('users.*') ? 'text-indigo-500 bg-indigo-100 font-medium' : 'text-gray-600 hover:bg-gray-100' }}">
                <i class="fas fa-user mr-2"></i> Users
              </a>
              <a href="{{ route('designations.index')}}" class="flex items-center px-2 py-2 text-sm rounded-md {{ request()->routeIs('designations.*') ? 'text-indigo-500 bg-indigo-100 font-medium' : 'text-gray-600 hover:bg-gray-100' }}">
                <i class="fas fa-id-badge mr-2"></i> Designations
              </a>
              <a href="{{ route('departments.index')}}" class="flex items-center px-2 py-2 text-sm rounded-md {{ request()->routeIs('departments.*') ? 'text-indigo-500 bg-indigo-100 font-medium' : 'text-gray-600 hover:bg-gray-100' }}">
      ...
      <div class="p-4 border-t border-gray-200">
        <div class="flex items-center">
          <img class="w-10 h-10 rounded-full" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="User profile">
          <div class="ml-3">
            <p class="text-sm font-medium text-gray-700">
              {{ auth()->user()->name }} 
              <span class="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full">{{ auth()->user()->designation->designation_name }}</span>
            </p>
